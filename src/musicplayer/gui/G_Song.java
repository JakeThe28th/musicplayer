...
public class G_Song implements G_Element {
	...
	public void draw(int left, int top, int right, int bottom, int depth) {
		...
		int text_width = API.size(name).x;

		if (text_width > (right-left) && (API.mouseY() > top && API.mouseY() < bottom)) {
			...
			API.text(left+time, top, depth, name);
			API.text((left-text_width)+time, top, depth, name);
